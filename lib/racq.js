...
var RacQ = function(options) {
	...
	this.claimMessages = function(queueName, parameters, callback) {
		...
		httpRequest('post', options, function(error, response, body) {
			if(error) {//error handle first
				return callback(error, null);
			} else {
				if(response.hasOwnProperty('statusCode')) {//make sure the response has the 'statusCode'
					if(response.statusCode === 201) {//we got data!!!!
						//debug('claimMessages: %s', util.inspect(body, true, 2));
						var claims = processClaims(body);
						return callback(null, claims);
					} else if(response.statusCode === 204) {//no data, but ok
						return callback(null, null);
					} else {//unexpected statusCode
						return callback(returnError('claimMessages', response.statusCode), null);
					}
				} else {//no statusCode and no error
					return callback(null, null);//same as 204
				}
			}
		});
	};
