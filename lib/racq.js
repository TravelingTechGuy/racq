...
var RacQ = function(options) {
	...
	// Claims  //
	/////////////
	this.claimMessages = function(queueName, limit, callback) {
		...
		request.post(options, function(error, response) {
			if(!error && (response.statusCode === 200 || response.statusCode === 204)) {
				...
				return callback(error || returnError('claimMessages', response.statusCode));
			}
		});
	};

	this.queryClaims = function(callback) {

	};

	this.updateClaims = function(callback) {

	};

	this.releaseClaims = function(callback) {
